...
def run_code(problem_id, code, language, user_id):
    ...
    lang_ext, lang_compiler = {'python': 'py', 'cpp': 'cpp', 'c': 'c'}, {'python': 'python', 'cpp': 'g++', 'c': 'gcc'}
...
def judge_submission(submission):
    ...
    files_to_remove = [code_file] + ([os.path.join(user_dir, f"sub_{submission.id}.out")] if submission.language in ['c', 'cpp'] else [])
...
def calculate_standings():
    ...
    standings = [{'username': user.username, 'solved': Submission.query.filter_by(user_id=user.id, status='Accepted').count()} for user in users]
